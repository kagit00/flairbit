...
public final class CsvExporter {
    ...
    public static <T> String exportToCsvString(List<T> entities, String groupIdentifier, List<FieldExtractor<T>> fieldExtractors) {
        if (entities.isEmpty()) throw new BadRequestException("Entities list cannot be null");
        if (StringUtils.isEmpty(groupIdentifier)) throw new BadRequestException("Group identifier cannot be null");
        if (Objects.isNull(fieldExtractors) || fieldExtractors.isEmpty()) throw new BadRequestException("Field extractors cannot be null or empty");
